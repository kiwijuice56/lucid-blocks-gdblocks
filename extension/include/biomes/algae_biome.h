...
class AlgaeBiome : public Biome {
	DECLARE_CLASS(AlgaeBiome, Biome);

	void generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate);
	int64_t get_block_at(Chunk* chunk, Vector3i position, int64_t ground_level, Ref<Noise> terrain_noise);
