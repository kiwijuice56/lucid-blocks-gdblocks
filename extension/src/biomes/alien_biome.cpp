...
DEFINE_CONSTRUCTORS_AND_BINDINGS(AlienBiome);

void AlienBiome::generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate) {
    Ref<Noise> terrain_noise = world->generator->noise[6];
    uint16_t rng_seed = world->generator->seed;
    int shale_number = world->block_name_map["shale block"];
    ...
    for (int64_t x = 0; x < Chunk::CHUNK_SIZE_X; x++) {
        ...
        for (int64_t y = 0; y < Chunk::CHUNK_SIZE_Y; y++) {
            ...
            int64_t block_type = get_block_at(chunk, chunk_position + Vector3i(x, y, z), biome_coordinate, 0, terrain_noise);

            // Sometimes spawn grass if block below is moss
            if (block_type == 0 && y > 0 && !block_locked(chunk, Vector3i(x, y - 1, z)) && Random::randf(rng_seed) < 0.05 && chunk->blocks[Chunk::position_to_index(Vector3i(x, y - 1, z))] == shale_number) {
                block_type = Random::randf(rng_seed) < 0.005 ? world->block_name_map["star foliage"] : world->block_name_map["balloon foliage"];
            }
...
int64_t AlienBiome::get_block_at(Chunk* chunk, Vector3i position, Vector3i biome_coordinate, int64_t ground_level, Ref<Noise> terrain_noise) {
    
    double terrain = terrain_noise->get_noise_3d(2.0 * position.x, position.y, 2.0 * position.z);
    ...
    if (terrain < 0.0) {
        ...
        return world->block_name_map["shale block"];
    } else  {
        return 0;
    }
}

int64_t AlienBiome::get_ground_level(Vector3i position, Vector3i biome_coordinate) {
