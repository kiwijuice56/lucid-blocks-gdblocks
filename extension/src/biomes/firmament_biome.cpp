...
DEFINE_CONSTRUCTORS_AND_BINDINGS(FirmamentBiome);

void FirmamentBiome::generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate)  {
    ...
    for (int64_t x = 0; x < Chunk::CHUNK_SIZE_X; x++) {
        ...
        if ((position.x % 4 == 0 || position.z % 4 == 0) && position.y == 0) {
            block_type = world->block_name_map["grass block"];
        }
        if ((position.x % 4 == 0 || position.z % 4 == 0) && position.y == 1 && Random::randf(rng_seed) < 0.07) {
            if (Random::randf(rng_seed) < 0.005) {
                block_type = world->block_name_map["plant"];
            } else {
                block_type = world->block_name_map["grass foliage"];
            }
            
        }
        if (position.x % 4 == 0 && position.z % 4 == 0 && position.y == 1 && Random::randf(rng_seed) < 0.006) {
