...
DEFINE_CONSTRUCTORS_AND_BINDINGS(ForestBiome);

void ForestBiome::generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate) {
    ...
    const double FOLIAGE_CHANCE = 0.25;
            ...
            if (position.y > ground_level + 1) {
                ...
                if (water_level < position.y && position.y <= water_level + beach_height + 1) {
                    ...
                    if (is_foliage_safe(chunk, position, local_position)) {
                        ...
                        if (Random::randf(rng_seed) < rare_chunk_chance) {
                            if (melon_chunk) {
                                block_type = position.y <= water_level ? 0 : (int) world->block_name_map["melon block"];
                            } else {
                                block_type = position.y <= water_level ? world->block_name_map["lively seaweed foliage"] : (Random::randf(rng_seed) < 0.8 ? (Random::randf(rng_seed) < 0.6 ? world->block_name_map["dandelion foliage"] : world->block_name_map["buzz flower foliage"]) : world->block_name_map["rose foliage"]);;
                            }
                        } else if (Random::randf(rng_seed) < FOLIAGE_RARE_CHANCE) {
                            block_type = position.y <= water_level ? world->block_name_map["lively seaweed foliage"] : world->block_name_map["aster foliage"];
...
void ForestBiome::generate_decorations(Vector3i chunk_position, Vector3i biome_coordinate) {
