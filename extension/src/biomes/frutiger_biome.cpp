...
DEFINE_CONSTRUCTORS_AND_BINDINGS(FrutigerBiome);

void FrutigerBiome::generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate) {
            ...
            if (position.y > ground_level + 1) {
                ...
                if (water_level < position.y && position.y <= water_level + beach_height + 1) {
                    ...
                    if (is_foliage_safe(chunk, position, local_position)) {
                        ...
                        if (Random::randf(rng_seed) < 0.06) {
                            block_type = position.y <= water_level ? world->block_name_map["seaweed foliage"] : world->block_name_map["grass grass foliage"];
                        }
                    }
                }
            } else if (position.y == ground_level || position.y == ground_level - 1 || position.y == ground_level - 2) {
                // Top level of ground
                int randomized_beach_height = beach_height + (int) (6 * sand_noise->get_noise_2d(position.x, position.z));
                block_type = position.y <= water_level + randomized_beach_height ? world->block_name_map["sand block"] : world->block_name_map["grass grass block"];
...
void FrutigerBiome::generate_decorations(Vector3i chunk_position, Vector3i biome_coordinate) {
