...
void GroundRustBiome::_bind_methods() {
    BIND_PROPERTY(GroundRustBiome, float, cutoff);
    BIND_PROPERTY(GroundRustBiome, float, decay);
}

DEFINE_CONSTRUCTORS(GroundRustBiome);

void GroundRustBiome::initialize() {
    Biome::initialize();
    rust_noise_1 = world->generator->noise[28];
}

void GroundRustBiome::generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate) {
    ...
    for (int64_t x = 0; x < Chunk::CHUNK_SIZE_X; x++) {
        ...
        for (int64_t y = 0; y < Chunk::CHUNK_SIZE_Y; y++) {
            ...
            if (block_type == 0) {
                float noise = rust_noise_1->get_noise_3dv(position);
                if (noise + decay * (position.y - ground_level) < cutoff) {
                    block_type = world->block_name_map["rust block"];
                }
            }
...
DEFINE_PROPERTY_GETTER_SETTER(GroundRustBiome, float, cutoff);
DEFINE_PROPERTY_GETTER_SETTER(GroundRustBiome, float, decay);

void GroundRustBiome::generate_decorations(Vector3i chunk_position, Vector3i biome_coordinate) {
