...
DEFINE_CONSTRUCTORS_AND_BINDINGS(LainBiome);

void LainBiome::generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate) {
                ...
                if (water_level < position.y && position.y <= water_level + beach_height + 1) {
                    ...
                    if (is_foliage_safe(chunk, position, local_position)) {
                        ...
                        if (Random::randf(rng_seed) < 0.015) {
                            if (melon_chunk) {
                                block_type = position.y <= water_level ? 0 : (int) world->block_name_map["flesh block"];
                            } else {
                                block_type = position.y <= water_level ? world->block_name_map["lively seaweed foliage"] : world->block_name_map["pale grass foliage"];
                            }
                        } else if (Random::randf(rng_seed) < 0.18) {
                            block_type = position.y <= water_level ? world->block_name_map["seaweed foliage"] : world->block_name_map["pale grass foliage"];
                        }
                    }
                }
            } else if (position.y == ground_level) {
                // Top level of ground
                block_type = position.y <= water_level + beach_height ? world->block_name_map["black sand block"] : world->block_name_map["pale grass block"];
...
void LainBiome::generate_decorations(Vector3i chunk_position, Vector3i biome_coordinate) {
