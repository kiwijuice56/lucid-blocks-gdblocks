...
DEFINE_CONSTRUCTORS_AND_BINDINGS(LucyBiome);

void LucyBiome::initialize() {
    Biome::initialize();
    pipe_noise = world->generator->noise[5];
}

void LucyBiome::generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate)  {
    ...
    for (int64_t x = 0; x < Chunk::CHUNK_SIZE_X; x++) {
        ...
        int key = (position.x + position.y + position.z) % 96 + 1;
        bool x_aligned = position.x % key == 0;
        bool y_aligned = position.y % key == 0;
        bool z_aligned = position.z % key == 0;
        ...
        if (aligned < 2) {
            ...
            if (presence_noise < +0.1) {
                ...
                block_type = world->block_name_map["cocoon block"];
                if (color_noise < +0.5) block_type = world->block_name_map["couch block"];
                if (color_noise < +0.07) block_type = world->block_name_map["comfort block"];
                if (color_noise < -0.25) block_type = world->block_name_map["patience block"];
            }
        }
...
void LucyBiome::generate_decorations(Vector3i chunk_position, Vector3i biome_coordinate) { }