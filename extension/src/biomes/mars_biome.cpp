...
DEFINE_CONSTRUCTORS_AND_BINDINGS(MarsBiome);

void MarsBiome::initialize() {
    Biome::initialize();
    ground_noise = world->generator->noise[32];
}

void MarsBiome::generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate) {
...
int64_t MarsBiome::get_block_at(Chunk* chunk, Vector3i position, Vector3i biome_coordinate, int64_t ground_level) {
    double terrain = ground_noise->get_noise_3d(position.x, position.y, position.z);
    ...
    if (terrain < 0.0) {
        ...
        return world->block_name_map["shoal rock block"];
    } else  {
        return 0;
    }
}

int64_t MarsBiome::get_ground_level(Vector3i position, Vector3i biome_coordinate) {
