#include "../../include/biomes/prison_biome.h"
#include "../../include/world.h"

#include <godot_cpp/core/class_db.hpp>
#include <godot_cpp/variant/utility_functions.hpp>

using namespace godot;

DEFINE_CONSTRUCTORS_AND_BINDINGS(PrisonBiome);

void PrisonBiome::generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate) {
    Dictionary empty_dictionary;

    uint16_t rng_seed = world->generator->seed;
    Random::scramble_rng_seed(&rng_seed, chunk_position);

    for (int64_t y = 0; y < Chunk::CHUNK_SIZE_Y; y++) {
    for (int64_t z = 0; z < Chunk::CHUNK_SIZE_Z; z++) {
    for (int64_t x = 0; x < Chunk::CHUNK_SIZE_X; x++) {
        int64_t water_level = get_water_level(chunk_position + Vector3i(x, 0, z), biome_coordinate);
        Vector3i position = chunk_position + Vector3i(x, y, z);
        Random::scramble_rng_seed(&rng_seed, position);

        if (block_locked(chunk, Vector3i(x, y, z))) {
            continue;
        }

        int32_t block_type = 0;

        if (position.z % CELL_SIZE == 0 || position.x % CELL_SIZE == 0 || position.y % CELL_HEIGHT == 0) {
            block_type = world->block_name_map["prismatic block"];
        }

        if (Random::randf(rng_seed) < 0.001 &&
        (int64_t) UtilityFunctions::abs(position.x % CELL_SIZE) == (CELL_SIZE / 2) &&
        (int64_t) UtilityFunctions::abs(position.z % CELL_SIZE) == (CELL_SIZE / 2) &&
        (int64_t) UtilityFunctions::abs(position.y % CELL_HEIGHT) == (CELL_HEIGHT - 1)) {
            block_type = world->block_name_map["lamp down block"];
        }

        if (is_on_vertical_border(chunk, position, Vector3i(x, y, z))) {
            block_type = block_type = world->block_name_map["prismatic block"];
        }

        if (is_on_vertical_border(chunk, chunk_position + Vector3i(x, y, z), Vector3i(x, y, z))) {
            block_type = world->block_name_map["prismatic block"];
        }

        chunk->blocks[Chunk::position_to_index(Vector3i(x, y, z))] = block_type;
        chunk->water[Chunk::position_to_index(Vector3i(x, y, z))] = position.y <= water_level ? 255 : 0;
    }
    }
    }
}

void PrisonBiome::generate_decorations(Vector3i chunk_position, Vector3i biome_coordinate) { }
