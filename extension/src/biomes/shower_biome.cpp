...
DEFINE_CONSTRUCTORS_AND_BINDINGS(ShowerBiome);

void ShowerBiome::generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate) {
    ...
    for (int64_t x = 0; x < Chunk::CHUNK_SIZE_X; x++) {
        ...
        for (int64_t y = 0; y < Chunk::CHUNK_SIZE_Y; y++) {
            ...
            if (real_y <= ground_level) {
                int mz = UtilityFunctions::posmod(position.z, 8);
                if (mz >= 4) mz = 6 - mz;
                int mx = UtilityFunctions::posmod(position.x + mz, 4);
                block_type = mx <= 1 ? floor_type_1 : floor_type_2;
            }
...
void ShowerBiome::generate_decorations(Vector3i chunk_position, Vector3i biome_coordinate) { }

int64_t ShowerBiome::get_ground_level(Vector3i position, Vector3i biome_coordinate) {
