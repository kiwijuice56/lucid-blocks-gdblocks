...
DEFINE_CONSTRUCTORS_AND_BINDINGS(SnowBiome);

void SnowBiome::generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate) {
    ...
    for (int64_t x = 0; x < Chunk::CHUNK_SIZE_X; x++) {
        ...
        for (int64_t y = 0; y < Chunk::CHUNK_SIZE_Y; y++) {
            ...
            if (position.y > ground_level + 1) {
                ...
                block_type = position.y <= water_level + randomized_beach_height ? world->block_name_map["stone block"] : world->block_name_map["snow grass block"];
...
void SnowBiome::generate_decorations(Vector3i chunk_position, Vector3i biome_coordinate) {
