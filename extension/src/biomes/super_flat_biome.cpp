...
DEFINE_CONSTRUCTORS_AND_BINDINGS(SuperFlatBiome);

void SuperFlatBiome::generate_chunk_data(Chunk* chunk, Vector3i chunk_position, Vector3i biome_coordinate) {
    ...
    for (int64_t x = 0; x < Chunk::CHUNK_SIZE_X; x++) {
        ...
        for (int64_t y = 0; y < Chunk::CHUNK_SIZE_Y; y++) {
            ...
            if (real_y <= ground_level && real_y >= ground_level - 8) {

                block_type = world->block_name_map["simblock 5"];
            }
...
int64_t SuperFlatBiome::get_ground_level(Vector3i position, Vector3i biome_coordinate) {
    return 0;
}

void SuperFlatBiome::generate_decorations(Vector3i chunk_position, Vector3i biome_coordinate) { }
