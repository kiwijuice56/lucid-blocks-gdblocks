#include "../../include/structures/clay_tower.h"
#include "../../include/biomes/fractal_biome.h"
#include "../../include/world.h"

#include <godot_cpp/core/class_db.hpp>
#include <godot_cpp/variant/utility_functions.hpp>

using namespace godot;

void ClayTower::_bind_methods() {
    BIND_REF_ARRAY_PROPERTY(ClayTower, Decoration, decorations);
}

DEFINE_CONSTRUCTORS(ClayTower);

void ClayTower::generate_chunk_data(Chunk* chunk, Vector3i chunk_position) {
    uint16_t rng_seed = world->generator->seed;

    for (int64_t i = 0; i < placed_decorations.size(); i++) {
        Ref<DecorationState> ds = placed_decorations[i];
        Ref<Decoration> d = ds->decoration;
        Generator::fill_decoration(world, chunk, chunk_position, d, ds, false);
    }
}

void ClayTower::initialize() {
    uint16_t rng_seed = world->generator->seed;

    Random::scramble_rng_seed(&rng_seed, heart_position);
    int i_offset = Random::randi(rng_seed) % 3 - 1;

    Random::scramble_rng_seed(&rng_seed, heart_position);
    int j_offset = Random::randi(rng_seed) % 3 - 1;

    for (int i = 3; i < World::STRUCTURE_SIZE / Chunk::CHUNK_SIZE_X - 3; i++) {
    for (int j = 3; j < World::STRUCTURE_SIZE / Chunk::CHUNK_SIZE_Z - 3; j++) {
        Vector3i column_position = root_position + Vector3i(Chunk::CHUNK_SIZE_X * (i + i_offset) + Chunk::CHUNK_SIZE_X / 2, World::STRUCTURE_SIZE / 2, Chunk::CHUNK_SIZE_Z * (j + j_offset) + Chunk::CHUNK_SIZE_Z / 2);

        Random::scramble_rng_seed(&rng_seed, column_position);
        column_position.x += Random::randi(rng_seed) % 10 - 5;

        Random::scramble_rng_seed(&rng_seed, column_position);
        column_position.z += Random::randi(rng_seed) % 10 - 5;

        Random::scramble_rng_seed(&rng_seed, Vector3i(i, 0, j));
        if (Random::randf(rng_seed) < 0.86) {
            continue;
        }

        // Pick center
        Ref<Biome> biome = world->generator->get_biome_at(column_position, Generator::LAYER_BASE);
        Vector3i biome_coordinate = world->generator->get_true_biome_coordinate(column_position);

        int64_t height_level = biome->get_ground_level(column_position, biome_coordinate);
        bool no_ground_level = height_level == Biome::GROUND_LEVEL_INVALID;

        if (no_ground_level) {
            Random::scramble_rng_seed(&rng_seed, column_position);
            height_level = root_position.y + Random::randi(rng_seed) % (World::STRUCTURE_SIZE / 2);
        }


        Vector3i placement_position = Vector3i(column_position.x, height_level, column_position.z);

        // Check that the 4 corners are OK
        bool skip = false;
        const int HEIGHT_ALLOWANCE = 6;
        const int BASE_CHECK_WIDTH = 6;
        for (int x = 0; x < 2; x++) {
        for (int z = 0; z < 2; z++) {
            Vector3i other_position = placement_position + BASE_CHECK_WIDTH * Vector3i(x == 0 ? -1 : 1, 0, z == 0 ? -1 : 1);
            Ref<Biome> biome = world->generator->get_biome_at(other_position, Generator::LAYER_BASE);
            Vector3i biome_coordinate = world->generator->get_true_biome_coordinate(other_position);

            if (no_ground_level) {
                skip = skip || !biome->is_solid_at(other_position);
            } else {
                int64_t other_height_level = biome->get_ground_level(other_position, biome_coordinate);
                if (UtilityFunctions::absi(other_height_level - height_level) > HEIGHT_ALLOWANCE) {
                    skip = true;
                }
            }
        }
        }

        if (skip) {
            continue;
        }

        // Stack tower parts together
        const int MIN_HEIGHT = 7;
        const int FLOOR_HEIGHT = 10;

        rng_seed -= world->generator->seed;
        Random::scramble_rng_seed(&rng_seed, placement_position);
        int64_t height = Random::randi(rng_seed) % 8;
        int block_height = (height + MIN_HEIGHT - 1) * 10 + 44; // Height of the tallest block in this tower

        // Prevent towers from being clipped
        if ((block_height + placement_position.y) - root_position.y >= World::STRUCTURE_SIZE) {
            continue;
        }

        rng_seed -= world->generator->seed * (i + 6);
        Random::scramble_rng_seed(&rng_seed, placement_position);
        int64_t direction = Random::randi(rng_seed) % 4;


        for (int i = 0; i < height + MIN_HEIGHT; i++) {
            Ref<DecorationState> d = memnew(DecorationState);

            if (direction == 0) {
                d->direction = DecorationState::North;
            } else if (direction == 1) {
                d->direction = DecorationState::East;
            } else if (direction == 2) {
                d->direction = DecorationState::South;
            } else if (direction == 3) {
                d->direction = DecorationState::West;
            }

            d->position = placement_position + Vector3(0, FLOOR_HEIGHT * i, 0);

            if (i == 0) {
                d->decoration = decorations[0];
            } else if (i == height + MIN_HEIGHT - 1) {
                d->decoration = decorations[2];
                if (!cutscene_block_decoration_spawned) {
                    d->has_cutscene_block = true;
                    cutscene_block_decoration_spawned = true;
                    cutscene_block_position = Generator::find_actual_block_position(
                            d->decoration->cutscene_block_position,
                            d->position, d->decoration->size, d->decoration->center_offset,
                            d->direction
                    );
                }

            } else {
                d->decoration = decorations[1];
            }

            placed_decorations.append(d);
        }

    }
    }
}

Vector3i ClayTower::get_cutscene_block_position() {
    return cutscene_block_position;
}

bool ClayTower::has_cutscene_block() {
    return cutscene_block_decoration_spawned;
}

bool ClayTower::is_within_structure(Vector3i position) {
    return false;
}

DEFINE_PROPERTY_GETTER_SETTER(ClayTower, TypedArray<Decoration>, decorations);
