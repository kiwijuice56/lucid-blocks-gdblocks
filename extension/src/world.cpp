...
void World::_bind_methods() {
    BIND_METHOD_NO_PARAMETERS(World, start_up);
    BIND_METHOD_NO_PARAMETERS(World, initialize);
    BIND_METHOD_NO_PARAMETERS(World, create_texture_atlas);
    BIND_METHOD_NO_PARAMETERS(World, clear);
    BIND_METHOD_NO_PARAMETERS(World, is_all_loaded);
    BIND_METHOD_NO_PARAMETERS(World, refresh_instance_radius);
    BIND_METHOD(World, set_loaded_region_center, "new_center");
    BIND_METHOD_NO_PARAMETERS(World, simulate_dynamic);
    BIND_METHOD(World, is_position_loaded, "position");
    BIND_METHOD(World, is_position_loading, "position");
    BIND_METHOD(World, snap_to_chunk, "position");
    BIND_METHOD(World, get_nearest_structure, "position");
    BIND_METHOD(World, is_within_structure, "position");
    BIND_METHOD(World, find_closest_cutscene_block, "starting_position", "collected_blocks");
    ...
    BIND_METHOD(World, load_region, "region");
    BIND_METHOD_NO_PARAMETERS(World, get_regions_to_save);
    BIND_METHOD(World, get_region_data, "coordinate");
    BIND_METHOD_NO_PARAMETERS(World, make_all_regions_dirty);

    BIND_METHOD_NO_PARAMETERS(World, get_block_types);
    BIND_METHOD(World, set_block_types, "new_block_types");

    BIND_METHOD_NO_PARAMETERS(World, get_decorations);
    BIND_METHOD(World, set_decorations, "new_block_types");

    BIND_METHOD(World, save_data, "data", "prefix");
    BIND_METHOD(World, load_data, "data", "prefix");

    BIND_METHOD(World, get_block_type_at, "position");
    BIND_METHOD(World, is_block_solid_at, "position");
    BIND_METHOD(World, break_block_at, "position", "play_effect", "override_restrictions");
    BIND_METHOD(World, explode_at, "position", "radius", "firey");
    ...
    BIND_METHOD(World, place_fire_at, "position", "amount");
    BIND_METHOD(World, get_fire_at, "position");
    BIND_METHOD(World, fire_eligible, "position");

    BIND_METHOD(World, register_living_block, "position", "living_block");
    BIND_METHOD(World, unregister_living_block, "position");
    BIND_METHOD(World, get_living_block_at, "position");

    BIND_METHOD_NO_PARAMETERS(World, set_block_indices);
    BIND_METHOD(World, set_fusion_table, "array", "width");
    BIND_METHOD_NO_PARAMETERS(World, force_reload);

    BIND_REF_PROPERTY(World, Generator, generator);
    BIND_REF_PROPERTY(World, ShaderMaterial, block_material);
    BIND_REF_PROPERTY(World, ShaderMaterial, foliage_material);
    BIND_REF_PROPERTY(World, ShaderMaterial, water_material);
    BIND_REF_PROPERTY(World, ShaderMaterial, water_surface_material);
    BIND_REF_PROPERTY(World, ShaderMaterial, transparent_block_material);
    BIND_PROPERTY(World, int64_t, instance_radius);
    BIND_REF_ARRAY_PROPERTY(World, ShaderMaterial, requires_texture_atlas);
...
void World::start_up() {
    ...
    is_block_non_collision.resize(block_types.size());
    ...
    for (int64_t i = 0; i < block_types.size(); i++) {
        ...
        is_block_living[index] = block->living_block_path != "";
        is_block_internal[index] = block->internal;
        is_block_top_face_randomized[index] = block->top_face_random;
        is_block_non_collision[index] = block->no_collision_block;
    }
...
void World::instantiate_chunks() {
    ...
    int existing_count = all_chunks.size();
    ...
    for (int i = existing_count; i < required_count; i++) {
        instantiate_chunk();
    }
...
Chunk* World::instantiate_chunk() {
    ...
    new_chunk->is_block_non_collision = is_block_non_collision;
...
void World::create_texture_atlas() {
    ...
    for (int64_t i = 0; i < block_types.size(); i++) {
        ...
        // texture array size, so we need one transparent image at the end so that nothing is rendered
...
void World::update_loaded_region() {
    elapsed_time = 0;
    frame_start_time = Time::get_singleton()->get_ticks_usec();
    ...
    if (finalizing_chunks) {
        while (elapsed_time < 3000) { //usec_budget_per_frame) {
            ...
            Chunk* chunk = init_queue[chunk_finalization_index];
            Vector3i coordinate = init_queue_positions[chunk_finalization_index];
            chunk->finalize_main_mesh_loading();
            chunk->finalize_water_mesh_loading();
            chunk->finalize_water_surface_mesh_loading();
            chunk->initialize_fire_visuals(coordinate);
    ...
    for (int i = 0; i < structured_vectors.size(); i++) {
        ...
        if (!is_chunk_in_radius(key, 3 * STRUCTURE_SIZE + instance_radius)) {
    ...
    if (all_structures_generated && all_decorations_generated) {
        ...
        bool any_chunks_unloaded = false;
        ...
        if (any_chunks_unloaded) {
            emit_signal("chunk_unloaded", Vector3i(0, 0, 0));   
        }

        if (init_queue.size() > 0) {
            chunk_finalization_index = 0;
            finalizing_chunks = true;

            has_task = true;
            int threads = UtilityFunctions::ceili(init_queue_positions.size() / (double) chunk_batch_size);
            task_id = WorkerThreadPool::get_singleton()->add_group_task(callable_mp(this, &World::initialize_chunk), threads);
        } else {
            all_loaded = true;
...
void World::unload_chunk(Chunk* chunk) {
    ...
    if (chunk_map.has(coordinate)) {
        ...
        if (chunk->modified) {
            ...
            if (chunk->dirty) dirty_regions[get_region_coordinate(coordinate)] = true;
        }
    }
}
...
void World::initialize_chunk(uint64_t index) {
    for (int i = index * chunk_batch_size; i < (index + 1) * chunk_batch_size; i++) {
        if (i >= init_queue.size()) {
            return;
        }
        Chunk* chunk = Object::cast_to<Chunk>(init_queue[i]);
        Vector3i coordinate = init_queue_positions[i];
        ...
        chunk->generate_water_mesh(true, coordinate, false);
        chunk->generate_water_surface_mesh(true, coordinate, false); // This CLEARS the surface mesh
    }
...
void World::simulate_dynamic() {
    ...
    for (int64_t chunk_z = 0; chunk_z <= 2 * water_simulate_radius / Chunk::CHUNK_SIZE_Z; chunk_z++) {
        ...
        if (chunk->water_updated > 0) {
            ...
            rendered_chunks++;
            chunk->generate_water_mesh(false, coordinate, true);
            chunk->generate_water_surface_mesh(false, coordinate, true);
            chunk->water_updated--;
...
bool World::is_position_loaded(Vector3 position) const {
...
bool World::is_position_loading(Vector3 position) const {
    position = position.floor();
    Vector3i snapped_position = snap_to_chunk(position);
    return is_chunk_loaded.has(snapped_position) && !is_chunk_loaded[snapped_position];
}
...
Vector3i World::snap_to_chunk(Vector3 position) const {
    position = position.floor();
...
Vector3i World::get_region_coordinate(Vector3 position) const {
    Vector3i p = snap_to_chunk(position);
    p /= Vector3i(Chunk::CHUNK_SIZE_X, Chunk::CHUNK_SIZE_Y, Chunk::CHUNK_SIZE_Z);
    int64_t rx = int64_t(p.x) % REGION_SIZE;
    int64_t ry = int64_t(p.y) % REGION_SIZE;
    int64_t rz = int64_t(p.z) % REGION_SIZE;
    if (rx < 0) rx += REGION_SIZE;
    if (ry < 0) ry += REGION_SIZE;
    if (rz < 0) rz += REGION_SIZE;
    return (p - Vector3i(rx, ry, rz)) / REGION_SIZE; // not really a snap
}

Vector3i World::snap_to_nearest_structure(Vector3 position) const {
    position = position.floor();
...
bool World::is_chunk_in_radius(Vector3i coordinate, int64_t radius) const {
    if (radius <= 112) {
        return (center_chunk - coordinate).length_squared() < radius * radius;
    } else { // Add a little bias for large radii since we don't usually need to see super high up
        return (Vector3(center_chunk - coordinate) * Vector3(1, 1.5, 1)).length_squared() < radius * radius;
    }
...
void World::break_block_at(Vector3 position, bool play_effect, bool override_restrictions) {
    ...
    emit_signal("block_broken", position, 1);
}
...
void World::explode_at(Vector3 position, int radius, bool firey) {
    ...
    emit_signal("block_broken", position, radius);
...
uint8_t World::get_water_level_at(Vector3 position) {
    position = position.floor();
    Chunk* chunk = get_chunk_at(position);
...
bool World::is_under_water(Vector3 position) {
    uint8_t water_level = get_water_level_at(position);
    if (water_level == 0) {
        return false;
    }
    ...
    if (is_position_loaded(position + Vector3i(0, 1, 0))) {
        above_water = get_water_level_at(position + Vector3i(0, 1, 0));
    }
...
// DEPRECATED: only called when converting qualia from before version 3.0.0
void World::load_data(Dictionary data, String prefix) {
    ...
    if (root.has(prefix + "chunk_water")) {
        chunk_water_data = TypedDictionary<Vector3i, PackedByteArray>(root[prefix + "chunk_water"]);
...
void World::register_loaded_chunks() {
    for (uint64_t i = 0; i < all_chunks.size(); i++) {
        Chunk* chunk = all_chunks[i];
        Vector3i coordinate = Vector3i(chunk->get_position());
        if (chunk->modified) {
...
void World::load_region(Dictionary region) {
    if (region.has("chunk_block")) {
        Dictionary mapped_chunk_data = region["chunk_block"];
        Array saved_coordinates = mapped_chunk_data.keys();
        for (uint64_t i = 0; i < mapped_chunk_data.size(); i++) {
            Vector3i coordinate = saved_coordinates[i];
            PackedInt32Array blocks_id = mapped_chunk_data[coordinate];
            PackedInt32Array blocks;
    ...
    if (region.has("chunk_water")) {
        TypedDictionary<Vector3i, PackedByteArray> region_data = region["chunk_water"].duplicate(true);
        TypedArray<Vector3i> saved_coordinates = region_data.keys();
        for (uint64_t i = 0; i < region_data.size(); i++) {
            Vector3i coordinate = saved_coordinates[i];
            chunk_water_data[coordinate] = region_data[coordinate];
        }
    }

    if (region.has("chunk_water_awake")) {
        TypedDictionary<Vector3i, PackedByteArray> region_data = region["chunk_water_awake"].duplicate(true);
        TypedArray<Vector3i> saved_coordinates = region_data.keys();
        for (uint64_t i = 0; i < region_data.size(); i++) {
            Vector3i coordinate = saved_coordinates[i];
            chunk_water_awake_data[coordinate] = region_data[coordinate];
        }
    }

    if (region.has("chunk_fire")) {
        TypedDictionary<Vector3i, PackedByteArray> region_data = region["chunk_fire"].duplicate(true);
        TypedArray<Vector3i> saved_coordinates = region_data.keys();
        for (uint64_t i = 0; i < region_data.size(); i++) {
            Vector3i coordinate = saved_coordinates[i];
            chunk_fire_data[coordinate] = region_data[coordinate];
        }
    }
}

Dictionary World::get_region_data(Vector3i region_coordinate) const {
    Dictionary region_file;
    TypedDictionary<Vector3i, PackedByteArray> block_data;
    TypedDictionary<Vector3i, PackedByteArray> water_data;
    TypedDictionary<Vector3i, PackedByteArray> water_awake_data;
    TypedDictionary<Vector3i, PackedByteArray> fire_data;

    Vector3i region_corner = REGION_SIZE * Vector3i(Chunk::CHUNK_SIZE_X, Chunk::CHUNK_SIZE_Y, Chunk::CHUNK_SIZE_Z) * region_coordinate;

    for (int x = 0; x < REGION_SIZE; x++) {
    for (int y = 0; y < REGION_SIZE; y++) {
    for (int z = 0; z < REGION_SIZE; z++) {
        Vector3i chunk_coordinate = region_corner + Vector3i(
            x * Chunk::CHUNK_SIZE_X,
            y * Chunk::CHUNK_SIZE_Y,
            z * Chunk::CHUNK_SIZE_Z
        );

        if (chunk_data.has(chunk_coordinate)) {
            PackedInt32Array blocks = chunk_data[chunk_coordinate];
            PackedInt32Array blocks_id;
            blocks_id.resize(blocks.size());
            for (int64_t i = 0; i < blocks.size(); i++) {
                int32_t index = blocks[i];
            ...
            block_data[chunk_coordinate] = blocks_id;
        }

        if (chunk_water_data.has(chunk_coordinate)) {
            water_data[chunk_coordinate] = chunk_water_data[chunk_coordinate].duplicate(true);
        }

        if (chunk_water_awake_data.has(chunk_coordinate)) {
            water_awake_data[chunk_coordinate] = chunk_water_awake_data[chunk_coordinate].duplicate(true);
        }

        if (chunk_fire_data.has(chunk_coordinate)) {
            fire_data[chunk_coordinate] = chunk_fire_data[chunk_coordinate].duplicate(true);
        }
    }
    }
    }

    region_file["chunk_block"] = block_data;
    region_file["chunk_water"] = water_data;
    region_file["chunk_water_awake"] = water_awake_data;
    region_file["chunk_fire"] = fire_data;

    return region_file;
}

void World::make_all_regions_dirty() {
    TypedArray<Vector3i> saved_coordinates = chunk_data.keys();
    for (uint64_t i = 0; i < saved_coordinates.size(); i++) {
        dirty_regions[get_region_coordinate(saved_coordinates[i])] = true;
    }
}

TypedDictionary<Vector3i, bool> World::get_regions_to_save() const {
...
void World::measure_elapsed_time() {
    elapsed_time = Time::get_singleton()->get_ticks_usec() - frame_start_time;
}


void World::set_instance_radius(int64_t new_radius) {
    instance_radius = UtilityFunctions::clampi(new_radius, 48, 256);
}

void World::refresh_instance_radius() {
    std::vector<Chunk*> new_all_chunks;
    for (int i = 0; i < all_chunks.size(); i++) {
        Chunk* chunk = all_chunks[i];
        bool unneeded = chunk->garbage || !is_chunk_in_radius(chunk->get_position(), instance_radius);
        if (unneeded) {
            if (!chunk->garbage) {
            ...
            new_all_chunks.push_back(chunk);
        }
    }
...
DEFINE_PROPERTY_GETTER_SETTER(World, Ref<ShaderMaterial>, water_surface_material);
DEFINE_PROPERTY_GETTER_SETTER(World, Ref<ShaderMaterial>, transparent_block_material);
DEFINE_PROPERTY_GETTER_SETTER(World, Ref<Generator>, generator);
DEFINE_PROPERTY_GETTER_SETTER(World, TypedArray<ShaderMaterial>, requires_texture_atlas);
DEFINE_PROPERTY_GETTER_SETTER(World, TypedArray<Block>, block_types);
DEFINE_PROPERTY_GETTER_SETTER(World, TypedArray<Decoration>, decorations);
